...
echo "before $input_file $CXXBRIDGE_CMD $output_dir"

for input_file in "$@"; do
echo "adasdasd $CXXBRIDGE_CMD"
  base_name=$(basename "$input_file" .rs)
  "$CXXBRIDGE_CMD" "$input_file" --header > "$output_dir/${base_name}.rs.h"
  "$CXXBRIDGE_CMD" "$input_file" > "$output_dir/${base_name}.rs.cpp"
done
